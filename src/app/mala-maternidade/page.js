      ...
      <div className="w-full max-w-3xl">
        ...
        <div className="space-y-6">
          {listData && categoryOrder.map((categoryId) => {
            ...
            if (!category) return null; // Garante que a categoria exista
                ...
                <div className="divide-y divide-slate-200 dark:divide-slate-700">
                  {category.items.map((item) => (
                    <div key={item.id} className="flex items-center group">
                      ...
                      <button onClick={() => removeItem(categoryId, item.id)} className="p-2 rounded-full text-slate-400 hover:bg-red-100 hover:text-red-500 dark:hover:bg-red-900/50 opacity-0 group-hover:opacity-100 transition-opacity">
                        <DeleteIcon />
                      </button>
                    </div>
                  ))}
