// src/app/lista-de-compras/page.js
"use client";
...
function AddItemForm({ onAddItem }) {
  ...
  return (
    <form
      ...
      className="flex items-center gap-2 pt-3 mt-3 border-t border-slate-200 dark:border-slate-700"
    >
      <input
        type="text"
        value={label}
...
export default function ShoppingListPage() {
  ...
  const categoryOrder = ["quarto", "roupas", "higiene", "passeio"];
              ...
              categoryOrder.map((categoryId) => {
                ...
                return (
                  <Card key={categoryId}>
                      ...
                      {category.items.map((item) => (
                        <div key={item.id} className="flex items-center group">
                          <label className="flex-grow flex items-center gap-4 p-3 cursor-pointer">
                            ...
                            />
                            <div
                              className={`w-6 h-6 border-2 rounded-md flex-shrink-0 flex items-center justify-center transition-colors ${checkedItems.includes(item.id) ? "bg-rose-500 border-rose-500" : "border-slate-400 dark:border-slate-500"}`}
                            >
                              <svg
                                className={`w-4 h-4 text-white transform transition-transform ${checkedItems.includes(item.id) ? "scale-100" : "scale-0"}`}
                                viewBox="0 0 24 24"
                                fill="none"
                                stroke="currentColor"
                                strokeWidth="4"
                            ...
                            <span
                              className={`flex-grow text-slate-700 dark:text-slate-300 ${checkedItems.includes(item.id) ? "line-through text-slate-400 dark:text-slate-500" : ""}`}
                            >
                              {item.label}
                            </span>
                          </label>
                          <button
                            onClick={() =>
                              handleDeleteRequest(categoryId, item.id)
                            }
                            className="p-2 rounded-full text-slate-400 hover:bg-red-100 hover:text-red-500 dark:hover:bg-red-900/50 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity"
                          >
                            <DeleteIcon />
                          </button>
                        </div>
                    ...
                    <AddItemForm
                      onAddItem={(label) => addItem(categoryId, label)}
                    />
                  </Card>
                );
              })}
