...
const bmiCategories = [
  { category: "Baixo Peso", range: "< 18.5", recommendation: "12.5 a 18 kg" },
  {
    ...
    range: "18.5 - 24.9",
    recommendation: "11.5 a 16 kg",
  },
  {
    category: "Sobrepeso",
    range: "25.0 - 29.9",
    recommendation: "7 a 11.5 kg",
  },
...
export default function WeightTrackerPage() {
  ...
  const handleSaveInitialData = async () => {
    ...
    if (!user || !parsedHeight || !parsedWeight) {
      toast.warn("Preencha a altura e o peso corretamente.");
      return;
    }
    if (parsedHeight <= 0 || parsedWeight <= 0) {
      toast.warn("Altura e peso devem ser valores positivos.");
      return;
    }
    if (parsedHeight < 100 || parsedHeight > 250) {
      toast.warn("Por favor, insira uma altura realista (entre 100 e 250 cm).");
      return;
    }
  ...
  const handleAddWeight = async () => {
    const parsedCurrentWeight = parseFloat(currentWeight);
    if (!user || !parsedCurrentWeight || !entryDate) {
      toast.warn("Preencha o peso e a data do registro.");
      return;
    }
    if (parsedCurrentWeight <= 0) {
      toast.warn("O peso deve ser um valor positivo.");
      return;
    }
    if (parsedCurrentWeight < 30 || parsedCurrentWeight > 300) {
      toast.warn("Por favor, insira um peso realista (entre 30 e 300 kg).");
      return;
    }
  ...
  return (
    <>
      <ConfirmationModal
        ...
        message="Tem certeza que deseja apagar este registro de peso?"
      />
            ...
            {isEditing ? (
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={handleWeightInput(setPrePregnancyWeight)}
                ...
                <button
                  ...
                  className="mt-4 w-full bg-indigo-600 text-white font-semibold py-2 px-6 rounded-lg hover:bg-indigo-700 transition-colors"
                >
                  Salvar Dados
                </button>
              </>
                ...
                <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <input
                      type="tel"
                      value={currentWeight}
                      onChange={handleWeightInput(setCurrentWeight)}
                      placeholder="Peso em kg (Ex: 62.50)"
                      className="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-md bg-transparent dark:text-slate-200"
                    />
                    <input
                      type="date"
                      value={entryDate}
                    ...
                    <button
                      ...
                      className="bg-rose-500 text-white font-semibold py-2 px-6 rounded-lg hover:bg-rose-600 transition-colors"
                    >
                      Adicionar
                    </button>
                  </div>
                    ...
                    {displayedHistory.map((entry) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="flex items-center justify-center p-2 rounded-full text-slate-400 hover:bg-red-100 hover:text-red-500 dark:hover:bg-red-900/50 transition-colors"
                        >
                          <svg
