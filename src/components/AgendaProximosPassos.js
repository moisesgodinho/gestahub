            ...
            <div key={`${item.type}-${item.id}`}>
              ...
              <div className={`p-4 rounded-lg flex items-center gap-4 transition-colors ${item.type === 'ultrasound' ? 'border-l-4 border-rose-500' : 'border-l-4 border-indigo-500'} bg-slate-100 dark:bg-slate-700/50`}>
                ...
                <div className="flex-grow">
                  ...
                  {item.type === 'ultrasound' && lmpDate && <p className="text-xs text-rose-500 dark:text-rose-400 font-medium">{`Janela ideal: ${formatDateDisplay(new Date(getUTCDate(lmpDate).setDate(getUTCDate(lmpDate).getDate() + item.startWeek * 7)))} a ${formatDateDisplay(new Date(getUTCDate(lmpDate).setDate(getUTCDate(lmpDate).getDate() + (item.endWeek * 7) + 6)))}`}</p>}
