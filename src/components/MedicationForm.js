...
export default function MedicationForm({ onSave, onCancel, medicationToEdit }) {
  ...
  useEffect(() => {
    if (medicationToEdit) {
      ...
      if (type === "INTERVAL") {
        setStartTime(
          medicationToEdit.doses && medicationToEdit.doses.length > 0
            ? medicationToEdit.doses[0]
            : ""
        );
  ...
  const handleSubmit = (e) => {
    ...
    if (
      ...
      doses.some((dose) => !dose || !dose.trim())
    ) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              value={frequency}
              onChange={(e) => {
                if (scheduleType !== "INTERVAL") setFrequency(e.target.value);
              }}
              min="1"
              max="10"
              ...
              className={`mt-1 w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-md bg-transparent dark:text-slate-200 ${
                scheduleType === "INTERVAL"
                  ? "bg-slate-100 dark:bg-slate-700/50 cursor-not-allowed"
                  : ""
              }`}
            />
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-wrap gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                >
                  <div
                    className={`w-2.5 h-2.5 bg-white rounded-full transform transition-transform ${
                      scheduleType === "FLEXIBLE" ? "scale-100" : "scale-0"
                    }`}
                  ></div>
                </div>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  name="scheduleType"
                  value="FIXED_TIMES"
                  checked={scheduleType === "FIXED_TIMES"}
                  onChange={() => setScheduleType("FIXED_TIMES")}
                  className="sr-only peer"
                />
                <div
                  className={`w-5 h-5 border-2 rounded-full flex-shrink-0 flex items-center justify-center transition-colors ${
                ...
                >
                  <div
                    className={`w-2.5 h-2.5 bg-white rounded-full transform transition-transform ${
                      scheduleType === "FIXED_TIMES" ? "scale-100" : "scale-0"
                    }`}
                  ></div>
                </div>
                <span className="text-sm text-slate-700 dark:text-slate-300">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                >
                  <div
                    className={`w-2.5 h-2.5 bg-white rounded-full transform transition-transform ${
                      scheduleType === "INTERVAL" ? "scale-100" : "scale-0"
                    }`}
                  ></div>
                </div>
                <span className="text-sm text-slate-700 dark:text-slate-300">
              ...
              {doses.map((dose, index) => (
                <div key={index}>
                  ...
                  <input
                    ...
                    placeholder={
                      scheduleType === "FLEXIBLE" ? "Ex: Após o almoço" : ""
                    }
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              >
                <div
                  className={`w-2.5 h-2.5 bg-white rounded-full transform transition-transform ${
                    durationType === "CONTINUOUS" ? "scale-100" : "scale-0"
                  }`}
                ></div>
              </div>
              <span className="text-sm text-slate-700 dark:text-slate-300">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              >
                <div
                  className={`w-2.5 h-2.5 bg-white rounded-full transform transition-transform ${
                    durationType === "DAYS" ? "scale-100" : "scale-0"
                  }`}
                ></div>
              </div>
              <span className="text-sm text-slate-700 dark:text-slate-300">
