      ...
      <div
        ...
        onClick={() =>
          dayAppointments.length > 0
            ? onViewAppointment(dayAppointments)
          ...
          {isMobile ? (
            dayAppointments.length > 0 && (
              <div className="flex gap-1">
                {dayAppointments.some((a) => a.type === "ultrasound") && (
                  <div className="w-2 h-2 bg-rose-500 rounded-full"></div>
                )}
                {dayAppointments.some((a) => a.type === "manual") && (
      ...
      {lmpDate && (
        <div className="mt-6 pt-4 border-t border-slate-200 dark:border-slate-700">
          <h4 className="text-md font-semibold text-slate-700 dark:text-slate-300 mb-2">
            Legenda das Janelas Ideais:
          </h4>
          <div className="flex flex-wrap gap-x-4 gap-y-2">
            <div className="flex items-center gap-2">
              <span
                className={`w-4 h-4 rounded-lg ${colorClasses.appointment.legend}`}
              ></span>
