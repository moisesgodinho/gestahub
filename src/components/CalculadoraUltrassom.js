...
export default function CalculadoraUltrassom({
  user,
  onSaveSuccess,
  onCancel,
}) {
    ...
    if (user) {
      const fetchExamData = async () => {
        ...
        if (docSnap.exists() && docSnap.data().gestationalProfile?.ultrasound) {
          const { examDate, weeksAtExam, daysAtExam } =
            docSnap.data().gestationalProfile.ultrasound;
  ...
  const handleSave = async () => {
    ...
    if (examDateObj.getTime() > todayUTC.getTime()) {
