    ...
    calendarDays.push(
      ...
      >
        ...
        <div className="flex-grow flex items-center justify-center -mt-2">
          {mood ? (
            <span className="text-2xl" title={mood.value}>
              {mood.label.split(" ")[0]}
            </span>
          ) : entry ? (
            <div
              className="w-2.5 h-2.5 bg-indigo-400 rounded-full"
              title="Registro de sintoma/nota"
            ></div>
          ) : (
            <div></div>
          )}
    ...
    <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-2xl shadow-xl mb-6">
      <div className="flex justify-between items-center mb-4">
        <button
          ...
          disabled={!canGoBack}
          className="p-2 rounded-full text-slate-500 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed"
        >
          <ChevronLeftIcon />
        </button>
      ...
      <div className="grid grid-cols-7 gap-1 text-sm">
        {["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"].map((day) => (
          <div
            key={day}
            className="font-semibold text-center text-slate-500 dark:text-slate-400 p-2"
          >
            {day}
          </div>
        ))}
