    ...
    return (
      <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-2xl shadow-xl mb-6 text-center">
        ...
        <p className="text-slate-500 dark:text-slate-400">
          Adicione registros ao seu diário por alguns dias para ver um resumo
          gráfico do seu humor e sintomas.
        </p>
      </div>
    );
  }
    ...
    <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-2xl shadow-xl mb-6">
      <div className="mb-4 flex border-b border-slate-200 dark:border-slate-700">
        <button
          ...
          className={`py-2 px-4 font-semibold transition-colors ${activeTab === "mood" ? "border-b-2 border-indigo-500 text-indigo-600 dark:text-indigo-400" : "text-slate-500 dark:text-slate-400 hover:text-indigo-500"}`}
        >
          Resumo de Humor
        </button>
        <button
          onClick={() => setActiveTab("symptoms")}
          className={`py-2 px-4 font-semibold transition-colors ${activeTab === "symptoms" ? "border-b-2 border-rose-500 text-rose-600 dark:text-rose-400" : "text-slate-500 dark:text-slate-400 hover:text-rose-500"}`}
        >
          Resumo de Sintomas
        </button>
      </div>
