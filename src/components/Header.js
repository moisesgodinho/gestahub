...
// Ícones SVG como componentes (permanecem os mesmos)
const SunIcon = (props) => (
  <svg
    {...props}
...
export default function Header() {
  ...
  return (
    <header className="w-full p-4">
      <div
        className={`container mx-auto max-w-3xl flex items-center ${user ? "justify-between" : "justify-end"}`}
      >
        {user && (
          <Link
            href="/"
            className="text-2xl font-bold text-rose-500 dark:text-rose-400"
          >
            GestaHub
          </Link>
        )}
