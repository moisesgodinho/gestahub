...
export default function AppointmentItem({ item, onToggleDone, onEdit, onDelete, idealWindowText }) {
  const [isExpanded, setIsExpanded] = useState(false);

  return (
    <div className={`p-4 rounded-lg flex items-center gap-4 transition-colors ${item.done ? 'border-l-4 border-green-500' : (item.type === 'ultrasound' ? 'border-l-4 border-rose-500' : 'border-l-4 border-indigo-500')} bg-slate-100 dark:bg-slate-700/50`}>
      <div className="flex-shrink-0">
        <label className="cursor-pointer" title={item.done ? "Marcar como pendente" : "Marcar como concluído"}>
            <input type="checkbox" checked={!!item.done} onChange={() => onToggleDone(item)} className="sr-only peer" />
            <div className={`w-6 h-6 border-2 rounded-md flex items-center justify-center transition-colors ${item.done ? 'bg-green-500 border-green-500' : 'border-slate-400 dark:border-slate-500'}`}>
        ...
        {item.notes && (
          <div className="text-xs text-slate-500 dark:text-slate-400 italic mt-1">
              ...
              {item.notes.length > 50 && (
                  <button onClick={() => setIsExpanded(!isExpanded)} className="text-indigo-600 dark:text-indigo-400 hover:underline font-semibold">
