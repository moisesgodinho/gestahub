...
export default function JournalHistory({ entries, onEdit, user }) {
  ...
  const filteredEntries = useMemo(() => {
    return entries.filter(entry => {
      ...
      if (symptomFilter && (!entry.symptoms || !entry.symptoms.includes(symptomFilter))) return false;
      if (textFilter && (!entry.notes || !entry.notes.toLowerCase().includes(textFilter.toLowerCase()))) return false;
      return true;
    });
  ...
  const availableMonths = useMemo(() => Object.keys(entriesGroupedByMonth), [entriesGroupedByMonth]);
  const currentMonthEntries = useMemo(() => entriesGroupedByMonth[availableMonths[currentMonthIndex]] || [], [entriesGroupedByMonth, availableMonths, currentMonthIndex]);
  ...
  if (!entries || entries.length === 0) {
    return (
        <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-2xl shadow-xl mt-6 text-center">
             <h2 className="text-2xl font-semibold text-slate-800 dark:text-slate-200 mb-4">Histórico do Diário</h2>
             <p className="text-slate-500 dark:text-slate-400">Nenhum registro encontrado ainda.</p>
        </div>
    );
  }
        ...
        {availableMonths.length > 0 ? (
            <div>
                ...
                <div className="flex justify-between items-center mb-4 p-2 rounded-lg bg-slate-100 dark:bg-slate-700/50">
                    <button onClick={handleNextMonth} disabled={currentMonthIndex >= availableMonths.length - 1} className="p-2 rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed">
                        <ChevronLeftIcon />
                    </button>
                    <h3 className="text-lg font-bold text-slate-700 dark:text-slate-200 capitalize">
                        {availableMonths[currentMonthIndex]}
                    </h3>
                    <button onClick={handlePreviousMonth} disabled={currentMonthIndex <= 0} className="p-2 rounded-full text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed">
                        <ChevronRightIcon />
                    </button>
                </div>
                    ...
                    {currentMonthEntries.map(entry => (
                      <div key={entry.id} className="bg-slate-100 dark:bg-slate-700/50 p-4 rounded-lg">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="font-bold text-lg text-slate-800 dark:text-slate-100">{new Date(entry.date).toLocaleDateString('pt-BR', { day: '2-digit', weekday: 'long', timeZone: 'UTC' })}</p>
                            <p className="font-semibold text-indigo-600 dark:text-indigo-400">{getMoodLabel(entry.mood)}</p>
                          </div>
                          <div className="flex gap-2">
                             <button onClick={() => onEdit(entry)} title="Visualizar/Editar" className="p-2 rounded-full text-slate-500 hover:bg-blue-100 hover:text-blue-600 dark:hover:bg-blue-900/50 transition-colors">
                        ...
                        {entry.notes && (
                          <div className="mt-2">
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400 italic whitespace-pre-wrap">{entry.notes}</p>
                          </div>
                        )}
                      </div>
                    ))}
